...
public class Oauth2UserService extends DefaultOAuth2UserService{
	...
	// → 토큰 수령 → 토큰을 통해 사용자 정보 취득 → loadUser메소드 자동 호출 → 사용자 정보를 커스터마이징
	...
	public OAuth2User loadUser(OAuth2UserRequest userRequest)
		...
		// 예시) userId = kakao_12345
		String userId = provider + "_" + providerId;
		String password = passwordEncoder.encode(oAuth2UserInfo.getName());
		String email = oAuth2UserInfo.getEmail();
		String role = "ROLE_USER";
		
		// 사용자가 이미 소셜로그인한 기록이 있는지 검사할 객체 생성
		User user;
		
		if(userRepository.findById(userId).isPresent()) { // isPresent() 값이 있으면
			// 이미 소셜 로그인한 기록 존재하면, 정보를 user 엔티티에 담아줌
			user = userRepository.findById(userId).get();
		} else {
			// 소셜 로그인한 기록 없으면, null리턴하여 회원가입 처리
			user =null;
		}
		...
		if(user == null) {
			...
			// 추가 입력사항이 있으면 User 엔티티를 가지고 추가 입력 페이지로 이동
